...
class DiseaseType {
    constructor(name, initialOrgan, hpReduction, spreadTime, diseaseProgression = 2, additionalEffects = null) {
        this.name = name;
...
class Disease {
    constructor(diseaseType, patient, spreadChance = 0.05) {
        ...
        this.affectedOrgans = this.patient.organs.filter(o => o.name === diseaseType.initialOrgan);
    ...
    spreadDisease() {
        ...
        if (this.spreadCounter >= this.diseaseType.spreadTime) {
            ...
            if (healthyOrgans.length > 0) {
                const newOrgan = healthyOrgans[Math.floor(Math.random() * healthyOrgans.length)];
...
function neurotoxicInfectionEffect(patient, affectedOrgans) {
    if (affectedOrgans.length > 1 && affectedOrgans[affectedOrgans.length - 1].name === "Liver") {
...
function cardiacConstrictionEffect(patient, affectedOrgans) {
    if (affectedOrgans.length >= 1 && affectedOrgans[affectedOrgans.length - 1].name === "Lungs") {
...
function hepaticDeteriorationEffect(patient, affectedOrgans) {
    if (affectedOrgans.length > 1 && affectedOrgans[affectedOrgans.length - 1].name === "Kidneys") {
        patient.organs.forEach(organ => {
...
const hepaticDeterioration = new DiseaseType(
    ...
    20,
    10,
    hepaticDeteriorationEffect
);

function cerebralSepsisEffect(patient, affectedOrgans) {
    const heart = patient.organs.filter(o => o.name === "Heart");
    if (heart.length > 0 && affectedOrgans.length > 1 && affectedOrgans[affectedOrgans.length - 1].name === "Heart") {
...
const renalVenom = new DiseaseType(
    "Renal Venom",
    "Kidneys",
    2,
    6,
...
function neurocardiacSyndromeEffect(patient, affectedOrgans) {
    if (affectedOrgans.length >= 2 && affectedOrgans[affectedOrgans.length - 1].name === "Heart") {
        const lungs = patient.organs.filter(o => o.name === "Lungs");
        if (lungs.length > 0) {
            patient.disease.spreadChance += 0.15; // Increases the spread chance by 15%
        }
    }
}

const neurocardiacSyndrome = new DiseaseType(
    "Neurocardiac Syndrome",
    "Brain",
    1,
    4,
    neurocardiacSyndromeEffect
);
...
function generateRandomDisease() {
    const name = `${randomChoice(['Pulmonary', 'Cardio', 'Neuro', 'Hepato', 'Nephro'])} ${randomChoice(['Decay', 'Rot', 'Infection', 'Blight', 'Plague'])}`;
    ...
    return new DiseaseType(name, initialOrgan, hpReduction, spreadTime, additionalEffect);
}
...
class Herb {
  ...
  toString() {
      return `Herb(name=${this.name}, effectDescription=${this.effectDescription}, known=${this.known})`;
  }
}
...
function dewrootEffect(patient) {
  // Increases the health points of the Heart by 6 and reduces the health points of the Lungs by 2
  patient.organs.forEach(function(organ) {
      if (organ.name === "Heart") {
          organ.treat(6);
...
function fireweedEffect(patient) {
  // Increases the health points of the Lungs by 6 and reduces the health points of the Liver by 2
  patient.organs.forEach(function(organ) {
      if (organ.name === "Lungs") {
          organ.treat(6);
...
function bittermintEffect(patient) {
  // Increases the health points of the Liver by 6 and reduces the health points of the Kidneys by 2
  patient.organs.forEach(function(organ) {
      if (organ.name === "Liver") {
          organ.treat(6);
...
function wolfsbaneEffect(patient) {
  // Increases the health points of the Kidneys by 6 and reduces the health points of the Brain by 2
  patient.organs.forEach(function(organ) {
      if (organ.name === "Kidneys") {
          organ.treat(6);
...
function glimmerFernEffect(patient) {
  ...
  if (diseasedOrgans.length > 0) {
      let randomOrgan = diseasedOrgans[Math.floor(Math.random() * diseasedOrgans.length)];
      randomOrgan.treat(4);
  }
}
...
function serpentsFangEffect(patient) {
  ...
  if (criticalOrgans.length > 0) {
      let randomOrgan = criticalOrgans[Math.floor(Math.random() * criticalOrgans.length)];
      randomOrgan.treat(10, true);
  }
}

function phoenixAshEffect(patient) {
  // Increases the health points of the organ with the lowest health points by 8 and reduces the health points of other organs by 3 each
...
let possibleEffects = [
  { description: "+4 HP to Heart.", chance: 0.5, effectFunction: moonrootEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "Stops disease progression for 10 time steps.", chance: 0.15, effectFunction: silvervineEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "+6 HP to Lungs -2 HP to Liver.", chance: 0.15, effectFunction: fireweedEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "+8 HP to the organ with lowest HP -3 to other organs.", chance: 0.05, effectFunction: phoenixAshEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "+1 HP to all organs.", chance: 0.15, effectFunction: crimsonCloverEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "+4 HP to Lungs.", chance: 0.5, effectFunction: sunleafEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "+2 HP to Heart and +2 HP to Liver.", chance: 0.25, effectFunction: mossbellEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "Fully heals a random critical organ.", chance: 0.05, effectFunction: serpentsFangEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "+6 HP to Kidneys -2 HP to Brain.", chance: 0.1, effectFunction: wolfsbaneEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "+6 HP to Liver -2 HP to Kidneys.", chance: 0.1, effectFunction: bittermintEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "+4 HP to Liver.", chance: 0.3, effectFunction: nightshadeBerryEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "+4 HP to Kidneys.", chance: 0.3, effectFunction: bloodthornEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "+2 HP to Lungs and +2 HP to Kidneys.", chance: 0.15, effectFunction: ironbarkSapEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "+4 HP to a random diseased organ.", chance: 0.1, effectFunction: glimmerFernEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "+2 HP to Brain and +2 HP to Heart.", chance: 0.15, effectFunction: whisperleafEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "+4 HP to Brain.", chance: 0.5, effectFunction: lunarBlossomEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "+6 HP to a random diseased organ.", chance: 0.1, effectFunction: basiliskScaleEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "+6 HP to Heart -2 HP to Lungs.", chance: 0.2, effectFunction: dewrootEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "Boosts the effect of the next potion by x2.", chance: 0.15, effectFunction: dragonscaleFungusEffect, finalFunction: noEffect, duration: BASE_DURATION },
  { description: "Cures a diseased organ fully.", chance: 0.05, effectFunction: giantsHeartrootEffect, finalFunction: noEffect, duration: BASE_DURATION },
];
...
function verdantVineEffect(patient) {
  ...
  serpentsFangEffect(patient); // Assuming serpentsFangEffect is defined elsewhere
}
...
function updateHerbMap() {
  ...
  herbMap["Celestial Fungus"] = new Herb({
      ...
      effectDescription: "Heals critical organs and prevents organs from getting critical for 10 turns.",
      chance: 0.0,
      effectFunction: noEffect,
...
class Environment {
  constructor(name, herbs) {
      this.name = name;
      this.herbs = herbs; // List of herbs that can be gathered in this environment
  ...
  gatherHerbs(numHerbs = 3) {
      ...
      const weightedHerbs = this.herbs.map((herb, index) => ({
          herb,
...
class HerbGathering {
    constructor() {
        ...
        this.justAcquired = Object.fromEntries(availableHerbs.map(herb => [herb, 0]));
    ...
    gather(environment, numHerbs = NUM_GATHER) {
        ...
        for (let herb of gatheredHerbs) {
            let totalHerbs = Object.values(this.inventory).reduce((acc, value) => acc + value, 0)
...
const complementaryRunes = { "A": "B", "B": "A", "C": "D", "D": "C" };
...
class Druid {
    ...
    discoverHerbEffect(herbName) {
        if (!herbMap[herbName].known) {  // Check if the effect was previously unknown
            ...
            this.msg.push(`You have discovered the effect of ${herbName}: ${herbMap[herbName].effectDescription}`);
        }
        ...
        villages.forEach(v => {
            v.patients.forEach(patient => {
                ...
                if (patient.isDead()) {
                    ...
                    this.msg.push(`Patient ${patient.id} in ${v.name} is cured. You gained a crystal.`);
                    this.crystals += 1;
                    v.curedCount += 1;
    ...
    updateCursorP(newValue) {
        this.cursorP = Math.max(0, Math.min(newValue, this.currentVillage().patients.length - 1));
    ...
    updateCursorMiasma(newValue) {
        this.cursorMiasma = Math.max(0, Math.min(newValue, this.currentVillage().miasmaPatches.length - 1));
    }
    ...
    setState(newState) {
        ...
        switch (newState) {
            ...
            case "TRAVELLING":
                ...
                break;
            default:
                if (["TREATING", "GATHERING_SPECIFIC", "DISCOVERING", "SELECT_POTION"].includes(newState)) {
                    this.cursor = 0;
                }
                break;
        }
    ...
    removeMiasma() {
        ...
        if (patch.strength === 0) {
            this.currentVillage().miasmaPatches = this.currentVillage().miasmaPatches.filter(p => p !== patch);
        }
    ...
    initCombinationMap() {
        ...
        const newHerbs = ["Solar Blossom", "Shadowthorn", "Celestial Fungus", "Verdant Vine", "Ironleaf"];
    ...
    generateRunesForHerbs() {
        for (let herb in herbMap) {
            const runes = Array.from({ length: 3 }, () => "ABCD"[Math.floor(Math.random() * 4)]);
...
class Patient {
  constructor(id) {
      ...
      const randomDiseaseType = diseaseRegistry[Math.floor(Math.random() * diseaseRegistry.length)];
  ...
  applyDrugs(druid) {
      this.drugsTaken.forEach(drug => {
          ...
          if (drug.daysRemaining <= 0) {
              ...
              this.drugsTaken = this.drugsTaken.filter(d => d !== drug); // Remove the drug if time is up
...
const firstNames = [
  "Aeliana", "Bran", "Caius", "Dara", "Elara", "Faelan", "Gwyn", "Hale",
  "Iona", "Jarek", "Kiera", "Lucan", "Mira", "Nolan", "Orin", "Phaedra",
  "Quinn", "Ronan", "Sera", "Talia"
];

const lastNames = [
  "Ashdown", "Briar", "Caskwell", "Dunwood", "Eldridge", "Fenwick",
  "Grimsbane", "Hawke", "Ironwood", "Jasper", "Ketteridge", "Larkspur",
  "Mosswood", "Northgate", "Oakhart", "Pinehurst", "Quintrell", "Ravenwood",
  "Stormrider", "Thorne"
];

const prefixes = [
  "Stone", "Oak", "River", "Wolf", "Raven", "Iron", "Bright", "Shadow",
  "Frost", "Wind", "Moon", "Sun", "Dark", "Whisper", "Eagle", "Moss",
  "Pine", "Silver", "Golden", "Thunder"
];

const suffixes = [
  "haven", "wood", "field", "brook", "hill", "stead", "vale", "shire",
  "crest", "ford", "glen", "hollow", "cliff", "gate", "meadow", "ridge",
  "grove", "stone", "fall", "reach"
];
...
class Village {
  ...
  initializeMiasmaPatches() {
      ...
      for (let i = 0; i < numPatches; i++) {
          const location = ["North Field", "South Lake", "East Woods", "West Hill"][Math.floor(Math.random() * 4)];
  ...
  calculateMiasmaStrength() {
      return this.miasmaPatches.reduce((total, patch) => total + patch.strength, 0);
  }
  ...
  createMiasmaPatch() {
      if (this.miasmaPatches.length >= 3) {
          return this.miasmaPatches[Math.floor(Math.random() * this.miasmaPatches.length)];
      }
  ...
  progress() {
      ...
      if (shouldSpread) {
          const newPatient = this.villagers[Math.floor(Math.random() * this.villagers.length)];
          this.patients.push(newPatient);
          this.villagers = this.villagers.filter(villager => villager !== newPatient);
      }
...
function displayMiasmaPatches(druid, village, screen, x = 0, y = 0) {
    ...
    for (let i = 0; i < village.miasmaPatches.length; i++) {
        ...
        const strengthStr = `  Strength: ${strengthIndicator} (${patch.strength}/5)`;
        console.log(`${locationStr.padEnd(25)} | ${strengthStr}`);
...
function displayPatients(druid, screen, x = 0, y = 0) {
    ...
    y += 1;
    writeToScreen(screen, x, y, `Miasma Strength: ${village.calculateMiasmaStrength()}`);
    y += 1;
    writeToScreen(screen, x, y, `Spread Chance: ${(village.calculateSpreadChance() * 100).toFixed(2)}%`);
    y += 2;
    ...
    const rgt = village.patients.length > druid.windowP + druid.windowPSize ? ">" : " ";
    ...
    if (village.patients.length <= druid.windowP) {
        druid.windowP = Math.max(0, village.patients.length - druid.windowPSize - 1);
    }
    const end = Math.min(druid.windowP + druid.windowPSize, village.patients.length);
    ...
    if (village.patients.length > 0) {
        ...
        for (let i = 0; i < numOrgans; i++) {
            ...
            for (let j = druid.windowP; j < end; j++) {
                const organStr = displayOrgan(druid, village.patients[j].organs[i], screen, x, y);
...
function displayInventory(druid, screen, x = 0, y = 0) {
    ...
    let i = 0;
    const total = zfill(Object.values(druid.herbGathering.inventory).reduce((a, b) => a + b, 0));

    for (const [herb, quantity] of Object.entries(druid.herbGathering.inventory)) {
        const herbName = herb + " ".repeat(20 - herb.length);
        const qty = zfill(quantity);
        const cursor_s = druid.cursor === i ? ">" : " ";
        const justAcquired = druid.herbGathering.justAcquired[herb] > 0 ? "+" + druid.herbGathering.justAcquired[herb] : "  ";
        const runes = herbMap[herb].revealedRunes.join('');
        const effectDescription = herbMap[herb].known ? herbMap[herb].effectDescription: "Unknown effect.";

        let inventoryLine = `${cursor_s} ${qty} ${justAcquired}   ${herbName} ${runes}   ${effectDescription}`;
    ...
    writeToScreen(screen, x, y, `  ${total} / ${druid.herbGathering.maxInventory}`);
    y += 1;
...
function displayControls(screen, x = 0, y = 0) {
    const s = "(t) Treat Patient, (l) Look Patients, (g) Gather Ingredients (d) Discard Herb (T) Travel (p) Create Potion (m) Remove Miasma \n";
...
async function handleEnter(druid) {
  switch (druid.gameState) {
    ...
    case "CHOOSE_ENVIRONMENT":
      ...
      druid.writeMsg(`Gathered ingredients in the ${env.name} for ${DAYS_TO_GATHER} days.`);
      break;
    case "REMOVE_MIASMA":
      druid.removeMiasma();
...
async function mainLoop(druid, key) {
  try {
